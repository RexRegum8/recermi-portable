...
export function AdminSettings() {
  ...
  const fetchUSDT = async () => {
    ...
    } catch (e) { alert('Error al obtener la tasa USDT') }
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-50 p-5 overflow-auto">
            ...
            <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={config.ivaPercent} onChange={(e) => updateConfig({ ivaPercent: Number(e.target.value) })}
                ...
                <div>
                  ...
                  <input type="number" value={30} disabled className="w-full bg-slate-800/30 border border-slate-700 rounded-xl px-3 py-3 text-sm font-mono text-slate-500" />
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4 items-end">
                  <div className="flex-1">
                    <label className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1.5 block">Logo URL (PNG/SVG)</label>
                    <input value={config.companyLogo || ''} onChange={(e) => updateConfig({ companyLogo: e.target.value })}
                  ...
                  <div className="w-16 h-16 bg-slate-800 border border-slate-700 rounded-xl flex items-center justify-center overflow-hidden">
                    {config.companyLogo ? <img src={config.companyLogo} className="max-w-full max-h-full" /> : <span className="text-xl">🖼️</span>}
                  </div>
                </div>
             ...
             <div className="flex justify-between items-center mb-6">
               ...
               <div className="flex gap-2">
                 <input value={config.catalogUrl || ''} onChange={(e) => updateConfig({ catalogUrl: e.target.value })} 
                   className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-1 text-xs outline-none w-64" placeholder="URL del Catálogo" />
                 <button onClick={() => window.open(config.catalogUrl, '_blank')} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-xl text-[10px] font-black tracking-widest shadow-lg">VER PÁGINA EN VIVO</button>
               </div>
             </div>
               ...
               {products.map(p => (
                 <div key={p.id} className="bg-slate-800/40 p-3 rounded-2xl border border-slate-800 flex items-center justify-between group hover:border-blue-500/30 transition-all">
                    <div className="flex items-center gap-4">
                       <span className="text-2xl opacity-60 group-hover:opacity-100 transition-opacity">{p.image}</span>
                       <div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-800/50 rounded-2xl border border-slate-800">
                  ...
                  <input type="number" value={config.ptsPer10Usd || 1} onChange={e => updateConfig({ ptsPer10Usd: parseInt(e.target.value) })}
      ...
      {/* Modales New/Edit User - Same as original but styled */}
      {showAddUser && (
