...
export interface CartItem {
  productId: string; name: string; sku: string; price: number; quantity: number; image: string
}
...
export function CustomerProvider({ children }: { children: ReactNode }) {
  ...
  const refreshProfile = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        const data = await resp.json()
        setCustomer({ id: data.id, name: data.name, email: data.email, phone: data.phone, address: data.address })
  ...
  const updateCartQty = (productId: string, qty: number) => setCart((prev) => qty <= 0 ? prev.filter((i) => i.productId !== productId) : prev.map((i) => i.productId === productId ? { ...i, quantity: qty } : i))
