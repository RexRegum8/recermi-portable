        ...
        {filtered.map((p) => (
          <div key={p.id} className="bg-slate-900/70 border border-slate-800 rounded-xl overflow-hidden hover:border-slate-700 transition-all group">
            ...
            <div className="px-3 pb-3">
              <button onClick={() => addToCart({ productId: p.id, name: p.name, sku: p.sku, price: p.price, image: p.image })}
...
function ProductDetail({ id, goBack, goCart }: { id: string; goBack: () => void; goCart: () => void }) {
  ...
  const handleAdd = () => {
    addToCart({ productId: p.id, name: p.name, sku: p.sku, price: p.price, image: p.image })
    setAdded(true)
...
function CartPage({ goCheckout, goHome }: { goCheckout: () => void; goHome: () => void }) {
  const { cart, removeFromCart, updateCartQty, cartTotal, clearCart } = useCustomer()
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 h-fit sticky top-20">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-slate-400">Subtotal</span><span className="font-mono">${cartTotal.toFixed(2)}</span></div>
...
function CheckoutPage({ goOrders, goHome }: { goOrders: () => void; goHome: () => void }) {
  const { customer, cart, cartTotal, placeOrder, config } = useCustomer()
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="space-y-5">
        ...
        {/* Summary */}
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5">
          <h3 className="font-bold text-sm mb-3">📦 Resumen del Pedido</h3>
          <div className="space-y-1.5">
            {cart.map((i) => (
              <div key={i.productId} className="flex justify-between text-xs"><span className="text-slate-400">{i.quantity}x {i.name}</span><span className="font-mono">${(i.price * i.quantity).toFixed(2)}</span></div>
            ))}
            <hr className="border-slate-700 my-2" />
            <div className="flex justify-between font-bold"><span>Total USD</span><span className="font-mono text-green-400">${cartTotal.toFixed(2)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-slate-400">Total Bs (BCV)</span><span className="font-mono">{(cartTotal * exchangeRate).toFixed(2)}</span></div>
          </div>
        </div>
