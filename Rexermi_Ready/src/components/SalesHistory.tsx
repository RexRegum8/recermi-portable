...
export function SalesHistory() {
  ...
  const handleClose = async () => {
    if (activeSession) {
      await closeSession(activeSession.id, closingBalance)
                ...
                <table className="w-full text-left text-sm">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-slate-800/90 text-slate-400 text-[9px] uppercase tracking-widest font-bold backdrop-blur-sm">
                      <th className="px-4 py-3">ID</th><th className="px-4 py-3">Hora</th><th className="px-4 py-3">Cajero</th><th className="px-4 py-3">Productos</th><th className="px-4 py-3">Método</th><th className="px-4 py-3 text-right">IVA</th><th className="px-4 py-3 text-right">Total USD</th><th className="px-4 py-3 text-right">Total Bs</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/40">
                    {dailySales.map((s) => (
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-3 mb-6">
               <div className="flex justify-between text-sm"><span className="text-slate-400">Fondo Inicial</span><span className="font-mono">${activeSession.openingBalance.toFixed(2)}</span></div>
               <div className="flex justify-between text-sm"><span className="text-slate-400">Total Ventas USD</span><span className="font-mono text-green-400">${dailyTotal.toFixed(2)}</span></div>
               <div className="flex justify-between text-sm font-bold border-t border-slate-800 pt-3"><span className="text-white">EFECTIVO ESPERADO</span><span className="font-mono text-blue-400">${(activeSession.openingBalance + dailyTotal).toFixed(2)}</span></div>
            </div>
            <div className="space-y-4">
              <div>
